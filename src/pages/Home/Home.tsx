    ...
    <div className="app-shell">
      <main className="mobile-page">
        ...
        <div className="page-content">
          ...
          <nav className="feature-grid" aria-label="Informasi pasar">{features.map((item) => <button type="button" key={item.label}><span className={`feature-icon ${item.tone}`}><img src={asset(item.icon)} alt="" /></span><b>{item.label.split("\n").map((line) => <span key={line}>{line}</span>)}</b></button>)}</nav>
